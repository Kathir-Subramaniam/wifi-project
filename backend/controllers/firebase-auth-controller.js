...
class FirebaseAuthController {
  async registerUser(req, res) {
    try {
      ...
      try {
        await prisma.users.create({ data: { firstName, lastName, email, firebaseUid, roleId: BigInt(5) } });
  ...
  loginUser(req, res) {
    ...
    signInWithEmailAndPassword(auth, email, password)
      .then((userCredential) => {
        ...
        if (idToken) {
          res.cookie('access_token', idToken, { httpOnly: true, sameSite: 'lax' });
          logger.info({ uid: userCredential.user.uid, email }, 'User logged in');
